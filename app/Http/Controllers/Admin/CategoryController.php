...
class CategoryController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $categories = $categories->orderBy('orderno')->paginate(getPaginate(200));
    ...
    for($i=0;$i<count($order);$i++){
         ...
         $landmaster =Category::find($order[$i]);
        $landmaster->orderno = $i+1;
        $landmaster->save();
    }
       
        $notification = 'Category order updated successfully.';
    ...
    public function store(Request $request, $id = 0) {
        ...
        if($id == 0){
            ...
            $category->header   = $request->header ? 1 : 0;
        ...
        $path = imagePath()['categorybanner']['path'];
        $size = imagePath()['categorybanner']['size'];
        ...
        $category->header   = $request->header ? 1 : 0;
        $category->home   = $request->home ? 1 : 0;
        $category->save();
