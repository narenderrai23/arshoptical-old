...
class AdminController extends Controller {
	...
	if($i>0){
        ProductGallery::where('id',$data[4])->update([ 'qty' => $data[6]]);
	...
	$i++;
         ...
         $notify[] = ['success', 'CSV Uploaded Sucessfully'];
	 return redirect()->route('admin.request.quantity')->withNotify($notify);
	}
	...
	public function QuantityDownload(){
	$headers = [
    ...
    foreach($list as $key =>$row){
        ...
        if ($key == 0) {
            array_push($arr, array('PRODUCTID','NAME', 'SKU', 'MODEL_NO','COLORID', 'COLOR_CODE', 'QTY'));
        }

        foreach($row->productGallery as $gallery){
   ...
   $callback = function() use ($arr) 
	...
	$pageTitle = 'Request Quantity';
        $admin     = Auth::guard('admin')->user();
        return view('admin.quantity.index', compact('pageTitle', 'admin'));
	}
