...
function showAmount($amount, $decimal = 2, $separate = true, $exceptZeros = false)
{
    ...
    return number_format(round($printAmount, 2), 2);
...
function getPaginate($paginate = 60)
{
    return $paginate;
}
...
function imagePath()
{
 ...
 $data['categorybanner'] = [
        'path' => 'assets/images/categorybanner',
        'size' => '1920x500',
    ];
    $data['product'] = [
        'thumb'   => [
...
function productPriceWeb($product)
{
	 $user=Auth::user();
if($user){
    $productprice=$product->price - userDiscount($product->price,$user);
}
else{
 $productprice=$product->price ;
}

    $discountPrice = showDiscountPrice($productprice, $product->discount, $product->discount_type);
   // dd($product->price.'=ud='.userDiscount($product->price,$user).'=pp='.$product->price.'=dp='.$discountPrice);
    if ($product->today_deals == 1) {
        $general = GeneralSetting::first();
        $discountPrice = showDiscountPrice($productprice, $general->discount, $general->discount_type);
    }
    if ($discountPrice < 0) {
        $discountPrice = 0;
    }
...
function productPrice($product,$user)
{
    $productprice=$product->price - userDiscount($product->price,$user);
...
function discountText($product, $general)
{
    ...
    if ($product->discount != 0) {
        if ($product->discount_type == 1) {
            $discount = $general->cur_sym . showAmount($product->price);
        } else {
            $discount = showAmount($product->price) . '%';
        }
    } else if ($product->today_deals == 1) {
        if ($general->discount_type == 1) {
            $discount = $general->cur_sym . showAmount($general->price);
        } else {
            $discount = showAmount($general->price) . '%';
        }
    }

    return "<span class='badge badge--discount'>
                $discount
            </span>";
}
...
function sendOrder($mobileno,$orderno, $status){
$msg='Your Order No. '.$orderno.' purchased online has been '.$status.'. For any other information please call our helpline number. ARSH OPTICAL';
sendMessage($mobileno,$msg);
}

function sendMessage($mobile,$msg)
{
        $apiKey = 'PvBpu2yDOyldfzaI';
        $senderId = 'ARSOPT';
        $number = $mobile;
        $message = $msg;
      
        $url = "https://www.bulksmsindia.app/V2/http-api.php";
...
function userDiscountWeb($price){
    $user=Auth::user();
    if($user){
        $finalAmt=($price * $user->discount / 100);
    }
    else
    {
        $finalAmt=($price);
...
function userDiscount($price,$user){
